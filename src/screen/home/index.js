...
const Home = ({ navigation }) => {
  ...
  const { featuredOfferData, featuredOffersLoading } = useSelector(
    (s) => s.offers
  );
  const { featuredBusinessData, featuredBusinessLoading } = useSelector(
    (s) => s.business
  );
      ...
      dispatch(getUser())
        ...
        .then((res) => {
          pushNotificationTokens = [
            ...(res?.pushNotificationTokens || []),
            userDeviceToken,
          ];
          pushNotificationTokens = uniqBy(
            pushNotificationTokens,
            function (tokens) {
              return tokens;
            }
  ...
  useEffect(() => {
    if (onGetRefresh && defaultHub?.id) {
      ...
      dispatch(
        getRewardWallet({ userID: userData?.id, hubID: defaultHub?.id })
      );
    }
  }, [onGetRefresh]);
  ...
  useEffect(() => {
    if (activeNotification && onNotificationData?.isNavigate) {
      navigation.navigate(
        onNotificationData?.type === "offer" ? "businessTab" : "pointsTab"
      );
    }
  }, [activeNotification]);
  ...
  useEffect(() => {
    if (requireRefresh && route?.params?.passData?.data?.hubID) {
      dispatch(
        getRewardWallet({
          userID: userData?.id,
          hubID: route?.params?.passData?.data?.hubID,
        })
      );
      dispatch(getFeaturedOffers(route?.params?.passData?.data?.hubID));
      dispatch(getFeaturedBusiness(route?.params?.passData?.data?.hubID))
        .then(unwrapResult)
        .then((response) => {
          ...
          <View paddingT-24>
              ...
              <Pressable
                ...
                onPress={() => {
                  navigation.navigate("offersTab", {
                    screen: "offersList",
                    params: { title: "Featured Offers", source: "homeTab" },
                  });
                }}
              >
            ...
            {featuredOffersLoading ? (
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <OfferCard item={item} loading={featuredOffersLoading} />
                )}
          ...
          <View paddingT-24>
              ...
              <Pressable
                ...
                onPress={() => {
                  navigation.navigate("businessTab", {
                    screen: "businessList",
                    params: { title: "Featured businesses", source: "homeTab" },
                  });
                }}
              >
            ...
            {featuredBusinessLoading ? (
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <BusinessCard item={item} onPressBusiness={onPressBusiness} />
                )}
                keyExtractor={(_, index) => index.toString()}
                showsHorizontalScrollIndicator={false}
                keyboardDismissMode={"on-drag"}
