      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, backgroundColor: Colors.gray50 }}>
        <View style={{ backgroundColor: Colors.yellow }}>
          <View style={styles.card}>
            ...
            <View row centerV style={styles.footer}>
              <Text fs14 lh20 primary700>Transfer credits to City</Text>
              <Image source={Images.arrowRight} style={styles.rightImg} />
            </View>
            <View style={styles.separator2} />
            <TouchableOpacity onPress={() => navigation.navigate("offers")} row centerV style={styles.footer}>
        ...
        <View style={{ backgroundColor: Colors.white, paddingHorizontal: 10 }}>
          <View row centerV flex>
            <Text fs12 lh18 gray500 center style={styles.tableTitle}>Reference</Text>
            <Text fs12 lh18 gray500 center style={styles.tableTitle}>Date</Text>
            <Text fs12 lh18 gray500 center style={styles.tableTitle}>Credit Amount</Text>
            <Text fs12 lh18 gray500 center style={styles.tableTitle}>Type</Text>
          </View>
            ...
            return (
              <TouchableOpacity key={reward?.id} onPress={() => navigation.navigate('rewardDetails', { rewardId: reward?.id })} >
                <View row centerV flex style={{ backgroundColor: index % 2 == 0 ? Colors.gray50 : Colors.white }} >
                  <Text fs14 lh20 gray500 center style={styles.tableBody}>{`#${index + 1}`}</Text>
                  <Text fs14 lh20 gray500 center style={styles.tableBody}>{moment(reward?.attributes?.createdAt).format('ll')}</Text>
                  <Text fs14 lh20 gray500 center style={styles.tableBody}>{reward?.attributes?.credits}</Text>
                  <View fs14 lh20 success700 center style={styles.tableBody}>
                    {
                      reward?.attributes?.rewardType == "credit" ?
                        <View center style={styles.typeText}>
                          <Image source={Images.star} style={{ width: 12, height: 12, tintColor: Colors.success700, marginRight: 4 }} />
                          <Text fs12 lh18 success700>
                            Reward
                          </Text>
                        </View> :
                        <View center style={styles.typeReward}>
                          <Image source={Images.gift} style={{ width: 12, height: 12, tintColor: Colors.blue700, marginRight: 4 }} />
...
const styles = StyleSheet.create({
  ...
  footer: { marginHorizontal: 24, marginVertical: 16, display: "flex", flexDirection: "row" },
  ...
  typeText: { display: "flex", flexDirection: "row", backgroundColor: "#ECFDF3", paddingVertical: 4, paddingHorizontal: 8, borderRadius: 16, marginRight: 5, minWidth: 50 },
  typeReward: { display: "flex", flexDirection: "row", backgroundColor: "#EFF8FF", paddingVertical: 4, paddingHorizontal: 8, borderRadius: 16, marginRight: 5, minWidth: 50 }
})